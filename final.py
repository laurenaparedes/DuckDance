...
map = np.zeros((3000, 3000), dtype = np.float64) # Replace None by a numpy 2D floating point array
waypoints = []
...
while robot.step(timestep) != -1 and mode != 'planner':
    ...
    for i, rho in enumerate(lidar_sensor_readings):
        ...
        if rho < LIDAR_SENSOR_MAX_RANGE:
            ...
            if int(wx * 30) > 2999 or int(wx * 30) < 0:
                pass
            
            elif int(wy * 30) > 2999 or int(wy * 30) < 0:
                pass
    ...
    if mode == 'manual':
        ...
        #print('autonomous after ifs')    
        # distanceError = math.sqrt((pose_x - x_values[state])**2 + (pose_y - y_values[state])**2)
